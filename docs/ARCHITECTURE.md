...
| `briefing/today_chart.py` | The Today chart's data: 4-hour candles folded from the archived 15-minute bars (Yahoo's for days after the archive), their EMA20/50, the block still forming (provisional, following the live price), the previous session's high/low, the band each pattern's next close would have to land in (with its distance from the price now), the days each pattern in play formed, and the live candle in a session | Anything the browser would have to compute; a band presented as a reason to trade |
